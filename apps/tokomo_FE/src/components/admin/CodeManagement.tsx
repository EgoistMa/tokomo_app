...
export function CodeManagement() {
  ...
  const handleExportCSV = () => {
    const csvContent = "data:text/csv;charset=utf-8," 
      + ["兑换码,有效天数,状态,使用者ID,使用时间"]
      .concat(generatedCodes.map(code => 
        `${code.code},${code.validDays},${code.used ? '已使用' : '未使用'},${code.usedBy || ''},${code.usedAt || ''}`
      ))
      .join("\n");
    ...
    link.setAttribute("download", "vip_codes.csv");
  ...
  const handleExportAllCSV = () => {
    ...
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", "all_vip_codes.csv");
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
  ...
  return (
    <div className="bg-white shadow rounded-lg p-6">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-semibold">生成的VIP兑换码</h3>
              <button
            ...
            <div className="space-y-3 mb-4">
              {generatedCodes.slice(0, 10).map((code, index) => (
                <div key={index} className="bg-gray-100 rounded shadow">
                  ...
                  <div className="text-sm text-gray-600 mt-1">有效天数: {code.validDays}</div>
                </div>
              ))}
      ...
      {showEditModal && editingCode && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h3 className="text-lg font-medium mb-4">编辑兑换码</h3>
            <form onSubmit={(e) => handleUpdateCode(e, parseInt(editingCode.id))}>
              ...
              <div>
                ...
                <input
                  type="number"
                  value={editingCode.validDays}
                  onChange={(e) => setEditingCode({ ...editingCode, validDays: parseInt(e.target.value) })}
                  className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                />
              </div>
              <div>
